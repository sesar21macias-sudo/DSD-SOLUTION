...
export interface AgentResult {
  reply: string
  newOrder?: { order_id: string; order_number: string; total: number; tenant_id: string; order_items: unknown[] }
}
...
async function executeTool(
  ...
  tenantId: string,
  phone: string
): Promise<{ result: string; orderData?: AgentResult['newOrder'] }> {
  ...
  if (toolName === 'get_menu') {
    ...
    if (!products || products.length === 0) return { result: 'No hay productos disponibles en este momento.' }
  ...
  if (toolName === 'create_order') {
    ...
    const orderItems = input.items.map(item => {
      ...
      return { id: uuidv4(), product_id: item.product_id, quantity: item.quantity, unit_price: price, subtotal: price * item.quantity }
    })
    ...
    if (error || !order) return { result: 'Error al crear la orden, intenta de nuevo.' }

    await supabase.from('order_items').insert(
      orderItems.map(i => ({ ...i, order_id: order.id, tenant_id: tenantId }))
    )
  ...
  if (toolName === 'get_order_status') {
    const { order_number } = toolInput as { order_number: string }
    ...
    if (!order) return { result: 'No encontré esa orden. Verifica el número e intenta de nuevo.' }
  ...
  if (toolName === 'check_ingredient_availability') {
    const { product_name } = toolInput as { product_name: string }
    ...
    if (!product || !product.is_active) return { result: `Lo siento, ${product_name} no está disponible en este momento.` }
...
*IMPORTANTE:* Nunca inventes precios. Siempre consulta get_menu() para precios reales.`
  ...
  let newOrder: AgentResult['newOrder'] | undefined
  ...
  while (response.stop_reason === 'tool_use') {
    ...
    for (const toolUse of toolUses) {
      const { result, orderData } = await executeTool(
        toolUse.name,
        toolUse.input as Record<string, unknown>,
        tenantId,
        phone
      )
      if (orderData) newOrder = orderData
